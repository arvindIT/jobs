    ...
    <thead>
        <tr>
            ...
            <th data-direction="desc" data-sortable="true" data-filterable="true" <?php
                if (isset($sortfield) && $sortfield == 'email_id')
                {
                    if ($sortby == 'asc') {
                ...
                ?> role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-sort="descending" aria-label="Rendering engine: activate to sort column ascending"><a href="javascript:void(0);" onclick="applysortfilte_contact('email_id', '<?php echo $sorttypepass; ?>')">
                <?php echo $this->lang->line('common_email_title') ?>
                </a>
            </th>
    ...
    if (!empty($datalist) && count($datalist) > 0)
    {
        ...
        foreach ($datalist as $row) { ?>
            <tr <?php if ($i % 2 == 1) { ?>class="bgtitle" <?php } ?> >
                ...
                <td class="hidden-xs hidden-sm"><?php echo !empty($row['email_id']) ? strtolower($row['email_id']) : '' ?></td>
