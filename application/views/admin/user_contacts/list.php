...
<div class="modal fade bs-upload-modal-sm" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-sm" style="width: 400px;">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span>
                </button>
                <h4 class="modal-title" id="myModalLabel2">Upload Profile Picture</h4>
            </div>
            <form id="upload-profile" method="post" action="<?= $this->config->item('admin_base_url') . $viewname.'/upload_profile' ?>" data-parsley-validate class="form parsley-form form-horizontal form-label-left" enctype="multipart/form-data">
                <div class="modal-bodys">
                    <div class="right col-xs-1"></div>
                    <div class="right col-xs-4 text-center">
                        <img src="<?= base_url();?>images/img.jpg" alt="" id="upload_profile_preview" class="img-circle img-responsive">
                    </div>
                    <div class="right col-xs-7" style="padding-top: 30px;">
                        <input type="file" name="user_profile" name="user_profile" onChange="showprofileimagepreview(this, this.id)" />
                    </div>
                </div>
...
<!--                        <select name="DataTables_Table_0_length" size="1" aria-controls="DataTables_Table_0" id="delete_all" onchange="deletepopup1()">
                    ...
                    <div class="x_content">
						<div class="row" id="list_user_profile">
...
<script>
    function contact_search(allflag)
    {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo base_url(); ?>admin/user_contacts/" + uri_segment,
            data: {
            ...
            beforeSend: function () {
                $('#list_user_profile').block({message: 'Loading...'});
            },
            success: function (html) {
                $("#list_user_profile").html('');
                $("#list_user_profile").html(html);
                $('#list_user_profile').unblock();
            }
        });
        return false;
    }
    ...
    function update_id(id , pic)
    {
        ...
        {
            var src = "<?= base_url('images/no_image.jpg')?>";
        }
        else
        {
            var src = "<?= $this->config->item('user_pic_small_img_url')?>"+pic;
        }
        $('#user_id').val('');
        $('#upload_profile_preview').val('');
        $('#user_id').val(id);
        $('#upload_profile_preview').attr('src' , src);
    }
</script>