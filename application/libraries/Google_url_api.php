...
class Google_url_api {
    ...
    function __construct() {
        ...
        $this->_obj->load->model('general_model');
        $this->_obj->load->config('google_url_api');
        $config_value = $this->_obj->general_model->select('setting_master', array('field,value'));
        if (!empty($config_value)) {
            $config_value = array_combine(array_map(function ($o) {
                        return $o['field'];
                    }, $config_value), $config_value);
        }
        if (!empty($config_value)) {

            $this->_api_url = !empty($config_value['google_api_url']['value']) ? $this->_obj->config->set_item('google_api_url', $config_value['google_api_url']['value']) : '';
            $this->_api_key = !empty($config_value['google_api_key']['value']) ? $this->_obj->config->set_item('google_api_key', $config_value['google_api_key']['value']) : '';
        }
