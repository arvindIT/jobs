...
class newsletter_management_control extends CI_Controller {
    ...
    public function index()
    {
        ...
        $data['sortfield'] = 'id';
        $data['sortby'] = 'desc';
        $searchsort_session = $this->session->userdata('newsletter_sortsearchpage_data');
        ...
        $config['base_url']         = site_url($this->user_type . '/' . "newsletter_management/");
        ...
        if (!empty($searchtext))
        {
            ...
            $match = array('email_id' => $searchkeyword);
            $data['datalist']       = $this->general_model->select('job_newsletter', '', $match, '', '=', '', $config['per_page'], $uri_segment, $sortfield, $sortby);
            $config['total_rows']   = $this->general_model->select('job_newsletter', '', $match, '', 'like', '', '', '', '', '', '', '1', '');
        } else {
            $data['datalist']       = $this->general_model->select('job_newsletter', '', '', '', '', '', $config['per_page'], $uri_segment, $sortfield, $sortby);
            $config['total_rows']   = $this->general_model->select('job_newsletter', '', '', '', '', '', '', '', '', '', '', '1', '');
        }
        ...
        $newsletter_sortsearchpage_data = array(
        ...
        $this->session->set_userdata('newsletter_sortsearchpage_data', $newsletter_sortsearchpage_data);
    ...
    public function insert_data()
    {
        ...
        /* data insert */
        $this->general_model->insert('job_newsletter', $cdata);
    ...
    public function edit_record()
    {
        ...
        $data['editRecord']     = $this->general_model->select('job_newsletter', '', array('id' => $id), '', '=');
        $data['main_content'] = "admin/" . $this->viewName . "/add";
        $this->load->view("admin/include/template", $data);
    }
    ...
    public function update_data()
    {
        ...
        /* Update data */
        $this->general_model->update('job_newsletter', $cdata, array('id' => $cdata['id']));
    ...
    public function ajax_delete_all() {
        ...
        if (!empty($id)) {
            $this->general_model->delete('job_newsletter', array('id' => $id));
        }
        ...
        for ($i = 0; $i < count($array_data); $i++) {
            $this->general_model->delete('job_newsletter', array('id' => $array_data[$i]));
            $delete_all_flag = 1;
            $cnt++;
        }
        //pagingation
    ...
    public function ajax_publish_all() {
        ...
        for ($i = 0; $i < count($array_data); $i++) {
            ...
            $this->general_model->update('job_newsletter', $cdata, array('id' => $array_data[$i]));
        }
        $searchsort_session = $this->session->userdata('user_sortsearchpage_data');
