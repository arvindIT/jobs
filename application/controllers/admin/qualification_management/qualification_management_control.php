...
class qualification_management_control extends CI_Controller {
    ...
    public function index() {
        ...
        $data['sortfield'] = 'id';
        $data['sortby'] = 'desc';
        $searchsort_session = $this->session->userdata('qualification_sortsearchpage_data');
        ...
        $config['base_url'] = site_url($this->user_type . '/' . "qualification_management/");
        ...
        if (!empty($searchtext)) {
            ...
            $data['datalist'] = $this->general_model->select('qualification_master', '', $match, '', '=', '', $config['per_page'], $uri_segment, $sortfield, $sortby);
            $config['total_rows'] = $this->general_model->select('qualification_master', '', $match, '', 'like', '', '', '', '', '', '', '1', '');
        } else {
            $data['datalist'] = $this->general_model->select('qualification_master', '', '', '', '', '', $config['per_page'], $uri_segment, $sortfield, $sortby);
            $config['total_rows'] = $this->general_model->select('qualification_master', '', '', '', '', '', '', '', '', '', '', '1', '');
        }
        $this->pagination->initialize($config);
        $data['pagination'] = $this->pagination->create_links();
        $data['msg'] = $this->message_session['msg'];

        $qualification_sortsearchpage_data = array(
        ...
        $this->session->set_userdata('qualification_sortsearchpage_data', $qualification_sortsearchpage_data);
    ...
    public function insert_data() {
        ...
        $cdata['priority'] = $this->input->post('priority');
        $cdata['created_by'] = $this->admin_session['id'];

        $this->general_model->insert('qualification_master', $cdata);
    ...
    public function edit_record() {
        ...
        $match = array('id' => $id);
        $result = $this->general_model->select('qualification_master', '', $match, '', '=');
    ...
    public function update_data() {
        ...
        /* Update data */
        $this->general_model->update('qualification_master', $cdata, array('id' => $cdata['id']));
    ...
    public function ajax_delete_all() {
        ...
        if (!empty($id) && $admin['id'] != $id) {
            $this->general_model->delete('qualification_master', array('id' => $id));
            unset($id);
        }
        ...
        for ($i = 0; $i < count($array_data); $i++) {
            $this->general_model->delete('qualification_master', array('id' => $array_data[$i]));
            $delete_all_flag = 1;
            $cnt++;
        }
        //pagingation
    ...
    public function ajax_publish_all() {
        ...
        for ($i = 0; $i < count($array_data); $i++) {
            ...
            $this->general_model->update('qualification_master', $cdata, array('id' => $array_data[$i]));
        }
        $searchsort_session = $this->session->userdata('user_sortsearchpage_data');
