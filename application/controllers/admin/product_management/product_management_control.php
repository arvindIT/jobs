...
class product_management_control extends CI_Controller {
    ...
    public function index()
    {
        ...
        $data['sortfield'] = 'id';
        $data['sortby'] = 'desc';
        $searchsort_session = $this->session->userdata('product_sortsearchpage_data');
        ...
        $config['base_url'] = site_url($this->user_type . '/' . "product_management/");
        ...
        if (!empty($searchtext)) {
            ...
            $data['datalist'] = $this->general_model->select('product_master', '', $match, '', '=', '', $config['per_page'], $uri_segment, $sortfield, $sortby);
            $config['total_rows'] = $this->general_model->select('product_master', '', $match, '', 'like', '', '', '', '', '', '', '1', '');
        } else {
            $data['datalist'] = $this->general_model->select('product_master', '', '', '', '', '', $config['per_page'], $uri_segment, $sortfield, $sortby);
            $config['total_rows'] = $this->general_model->select('product_master', '', '', '', '', '', '', '', '', '', '', '1', '');
        }
        $this->pagination->initialize($config);
        $data['pagination'] = $this->pagination->create_links();
        $data['msg'] = $this->message_session['msg'];

        $product_sortsearchpage_data = array(
        ...
        $this->session->set_userdata('product_sortsearchpage_data', $product_sortsearchpage_data);
    ...
    {
        $cdata['product_name'] = $this->input->post('product_name');
        $cdata['priority'] = $this->input->post('priority');
        $cdata['created_by'] = $this->admin_session['id'];

        $this->general_model->insert('product_master', $cdata);
    ...
    public function edit_record() {
        ...
        $match = array('id' => $id);
        $result = $this->general_model->select('product_master', '', $match, '', '=');
    ...
    public function update_data()
    {
        ...
        /* Update data */
        $this->general_model->update('product_master', $cdata, array('id' => $cdata['id']));
    ...
    {
        $id = $this->uri->segment(4);
        $this->general_model->delete('product_master', array('id' => $id));
    ...
    public function ajax_delete_all()
    {
        ...
        for ($i = 0; $i < count($array_data); $i++) {
            $this->general_model->delete('product_master', array('id' => $array_data[$i]));
            $delete_all_flag = 1;
            $cnt++;
        }
        //pagingation
    ...
    public function ajax_publish_all() {
        ...
        for ($i = 0; $i < count($array_data); $i++) {
            ...
            $this->general_model->update('product_master', $cdata, array('id' => $array_data[$i]));
        }
        $searchsort_session = $this->session->userdata('user_sortsearchpage_data');
