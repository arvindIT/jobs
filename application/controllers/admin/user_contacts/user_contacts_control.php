...
class user_contacts_control extends CI_Controller {
    ...
    public function index() {
        ...
        $data['sortfield'] = 'id';
        $data['sortby'] = 'desc';
        $searchsort_session = $this->session->userdata('user_contacts_sortsearchpage_data');
        ...
        $config['base_url'] = site_url($this->user_type . '/' . "user_contacts/");
        ...
        $user_contacts_sortsearchpage_data = array(
        ...
        $this->session->set_userdata('user_contacts_sortsearchpage_data', $user_contacts_sortsearchpage_data);
    ...
    public function new_user() {
        ...
        $config['base_url'] = site_url($this->user_type . '/' . "user_contacts/new_user/");
        $config['is_ajax_paging'] = TRUE; // default FALSE
        $config['paging_function'] = 'ajax_paging'; // Your jQuery paging
        if (!empty($allflag) && ($allflag == 'all' || $allflag == 'changesorting' || $allflag == 'changesearch')) {
            $config['uri_segment'] = 0;
    ...
    public function insert_data() {
        ...
        $cdata['post_code'] = $this->input->post('post_code');
        $cdata['phone_no'] = $this->input->post('phone_no');
        $cdata['birth_date'] = trim(date('Y-m-d', strtotime($this->input->post('birth_date'))));
