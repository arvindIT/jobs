...
class User_control extends CI_Controller {
    ...
    public function index($REFERRED_BY = 0) {
        $this->load->library('linkedin', array(
            'access' => "78xgaxug6nb4o1", //"<your Consumer Key / API Key goes here>",
            'secret' => "ElrTjlu6osJUMLIV", //"<your Consumer Secret / Secret Key goes here>",
            'callback' => site_url()."/apiLinkedin/receiver"//"<write here your site name>/receiver"
        ));
    ...
    public function receiver() {
        ...
        if (isset($_REQUEST['oauth_verifier'])) {
            $userData = $this->linkedin->getUserInfo(
                    serialize($this->linkedin->request_token), $this->session->userdata('oauth_verifier'), $this->session->userdata('oauth_access_token')
            );
        } else {
            $userData['status'] = 404;
        }
